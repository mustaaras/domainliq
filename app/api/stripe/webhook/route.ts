...
async function handleCheckoutCompleted(session: any) {
    ...
    try {
        ...
        console.log(`[Stripe Webhook] Seller notification email sent for order ${order.id}`);

    } catch (error) {
        console.error('[Stripe Webhook] Failed to send seller email:', error);
    }
    ...
    try {
        ...
        console.error('[Stripe Webhook] Failed to send buyer email:', error);
    }
}
